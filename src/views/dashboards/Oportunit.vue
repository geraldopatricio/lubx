...
const fetchDashboardData = async () => {
    ...
    try {
        ...
        if (resSpecs.data.data) {
            // AJUSTE: Forçando valores a 0 sempre que carregar novos dados para garantir preenchimento manual
            tableData.value = (resSpecs.data.data.itens || []).map(row => ({
                ...row,
                suaVendaEstimada: 0,
                gapLitros: row.potencialLitros,
                share: '0.00'
            }));
        }
...
const updateMapHighlight = () => {
    ...
    // AJUSTE: Movimento dinâmico e suave (FlyTo e fitBounds com duração profissional)
    if (selectedUF.value === 'Todos') {
        map.value.flyTo({ center: [-52, -15], zoom: 2.8, duration: 1500, essential: true });
