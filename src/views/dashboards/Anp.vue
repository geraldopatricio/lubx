...
// Indicadores Financeiros Expandidos (MUDANÇA 4)
...
// Função auxiliar para truncar texto (MUDANÇA 3)
...
const anosExibicao = computed(() => {
  ...
  const anos = [];
  if (inicio <= fim) {
    for (let i = inicio; i <= fim; i++) { anos.push(i); }
  }
  return anos;
});
...
async function fetchIndicadores() {
  try {
    const res = await fetch('https://economia.awesomeapi.com.br/last/USD-BRL,EUR-BRL,BTC-BRL,XAU-BRL');
    const data = await res.json();
    indicadoresFinanceiros.usd = parseFloat(data.USDBRL.bid).toLocaleString('pt-BR', { minimumFractionDigits: 3 });
    indicadoresFinanceiros.eur = parseFloat(data.EURBRL.bid).toLocaleString('pt-BR', { minimumFractionDigits: 3 });
    indicadoresFinanceiros.btc = parseFloat(data.BTCBRL.bid).toLocaleString('pt-BR', { maximumFractionDigits: 0 });
    indicadoresFinanceiros.ouro = parseFloat(data.XAUBRL.bid).toLocaleString('pt-BR', { minimumFractionDigits: 2 });
      ...
      <div class="custom-select" @click.stop="showPeriodoModal = !showPeriodoModal">
        ...
        <div v-if="showPeriodoModal" class="periodo-modal" @click.stop>
          <div class="range-inputs">
            <div><label>Início</label><input type="number" v-model.number="anoInicial" min="2010" max="2026"></div>
            <div><label>Fim</label><input type="number" v-model.number="anoFinal" min="2010" max="2026"></div>
          </div>
    ...
    <div class="main-content">
      <section class="table-container">
        ...
        <table class="anp-table">
          ...
          <tbody v-else>
            <tr><td :colspan="anosExibicao.length + 1" style="padding: 40px; text-align: center; color: #888;">Buscando dados da API...</td></tr>
          </tbody>
        </table>
      ...
      <aside class="lubx-news">
        ...
        <div class="news-card-dark">
          ...
          <!-- ABA HOMOLOGAÇÕES (MUDANÇAS 1, 2 e 3 IMPLEMENTADAS) -->
          <div v-if="activeSidebarTab === 'homologacoes'">
            ...
            <div v-for="(prod, i) in newsHomologacoes" :key="i" class="homolog-card-v2">
              <div class="card-date-v2">Registrado em {{ prod.ano_limpo }}</div>
              ...
              <div class="card-grid">
                ...
                <div class="grid-item">
                  ...
                  <div class="detentor-badge-v3">
                    <span class="dot" :class="prod.detentorNome !== 'Uso Geral' ? 'yellow' : 'green'"></span>
                    <strong class="val-white">{{ prod.detentorNome }}</strong>
                  </div>
                </div>
                <div class="grid-item">
                  <i class="icon-gray">📝</i> <span class="label-gray">Desempenho</span>
                  <!-- MUDANÇA 3: Truncado em 50 com Hint no hover -->
                  <strong class="val-white hint-text" :title="prod.desempenho_original">
                    {{ truncateText(prod.desempenho_original, 50) }}
                  </strong>
                </div>
                <div class="grid-item">
                  <i class="icon-gray">📅</i> <span class="label-gray">Ano</span>
                  <strong class="val-white">{{ prod.ano_limpo }}</strong>
                </div>
              </div>
            </div>
          </div>

          <!-- ABA INDICADORES (MUDANÇA 4 IMPLEMENTADA) -->
          <div v-if="activeSidebarTab === 'indicadores'" class="indicators-view">
             <div class="indicador-header-lumen">
               <span class="lumen-tag">📊 MERCADO FINANCEIRO</span>
               <span class="lumen-date">Cotações em tempo real</span>
             </div>
             <div class="ind-line"><span>Moeda</span> <strong>USD/BRL</strong> <span>R$ {{ indicadoresFinanceiros.usd }}</span></div>
             <div class="ind-line"><span>Moeda</span> <strong>EUR/BRL</strong> <span>R$ {{ indicadoresFinanceiros.eur }}</span></div>
             <div class="ind-line"><span>Crypto</span> <strong>BTC/BRL</strong> <span>R$ {{ indicadoresFinanceiros.btc }}</span></div>
             <div class="ind-line"><span>Metal</span> <strong>Ouro (g)</strong> <span>R$ {{ indicadoresFinanceiros.ouro }}</span></div>
             <div class="ind-line"><span>Commodity</span> <strong>Petróleo Brent</strong> <span>$ {{ indicadoresFinanceiros.brent }}</span></div>
          </div>
        </div>
      </aside>
    </div>
...
.anp-table td { padding: 4px 8px; text-align: center; border-bottom: 1px solid #f9f9f9; font-size: 12px; height: 35px; min-width: 65px; }
...
.sub-row td { padding: 3px 8px; height: 30px; }
...
.detentor-badge-v3 { display: flex; align-items: center; gap: 6px; justify-self: end; background: #2a2a2a; padding: 2px 8px; border-radius: 4px; max-width: 160px; overflow: hidden; }
.detentor-badge-v3 strong { font-size: 9px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.hint-text { cursor: help; border-bottom: 1px dotted #555; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; display: block; max-width: 150px; }
...
.ind-line { display: grid; grid-template-columns: 1fr 1.5fr 1.5fr; align-items: center; border-bottom: 1px solid #222; padding: 8px 0; font-size: 10px; color: #888; }
.ind-line strong { color: #f58220; }
.ind-line span:last-child { text-align: right; color: #fff; font-weight: bold; }
