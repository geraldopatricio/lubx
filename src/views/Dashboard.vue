...
// Função central que retorna todos os parâmetros ativos para qualquer chamada de API
const getParams = () => {
    return {
...
const loadData = async () => {
    ...
    try {
        ...
        const endpoint = viewMode.value === 'executive' ? '/panorama' : '/analise-detalhada';
...
const openDetailedModal = async (type) => {
    ...
    try {
        const params = getParams(); // PEGA TODOS OS FILTROS DO TOPO
...
// --- RESTANTE DA LÓGICA DE GRÁFICOS ---
...
const clearFilters = () => { 
    Object.keys(filters).forEach(k => { 
        if (!k.startsWith('troca')) filters[k] = 'Todos'; 
    }); 
};
...
const chartCidadesTop5 = computed(() => {
    ...
    return { 
        ...
        datasets: [{ backgroundColor: '#e97332', data: list.slice(0,5).map(c => c.litrosAno/1000000), barThickness: 15 }] 
    };
});
...
const chartCidadesFull = computed(() => {
    ...
    return { 
        ...
        datasets: [{ backgroundColor: '#e97332', data: list.slice(0,30).map(c => c.litrosAno/1000000), barThickness: 12 }] 
    };
});
...
const chartViscosidade = computed(() => {
    ...
    return {
        ...
        datasets: [
            ...
            { type: 'bar', label: 'Vol Mi', backgroundColor: '#3b82f6', data: items.map(i => i.litros / 1000000), yAxisID: 'y', barThickness: 28 }
        ]
    };
});

const chartEstados = computed(() => ({ labels: detalhada.value?.graficos?.estado?.slice(0,10).map(e=>e.estado) || [], datasets: [{ backgroundColor: '#e97332', data: detalhada.value?.graficos?.estado?.slice(0,10).map(e=>e.litrosAno/1000000) || [], barThickness: 25 }] }));
const chartEstadosFull = computed(() => ({ labels: detalhada.value?.graficos?.estado?.map(e=>e.estado) || [], datasets: [{ backgroundColor: '#e97332', data: detalhada.value?.graficos?.estado?.map(e=>e.litrosAno/1000000) || [], barThickness: 15 }] }));
                ...
                <div class="row g-3 mb-4">
                    <div v-for="k in detalhada.cardsTopo" :key="k.id" class="col-md-3">
                        <div class="card border-0 shadow-sm p-4 rounded-4 bg-white d-flex flex-row align-items-center justify-content-center gap-3" :class="{'card-white-kpi': k.id === 'marcas' || k.id === 'modelos'}" @click="(k.id === 'marcas' || k.id === 'modelos') ? openDetailedModal(k.label) : null">
                            ...
                            <div>
                                <h2 class="fw-bold m-0 text-dark" style="font-size:1.7rem">{{ k.id === 'litros-ano' ? formatNum(totalLitrosCalculado) : formatNum(k.valor) }}</h2>
    ...
    <div v-if="showModal" class="modal-overlay d-flex align-items-center justify-content-center" @click.self="showModal = false">
        <div class="modal-content bg-white p-5 rounded-4 shadow-lg position-relative" :style="{ width: (modalChartType === 'bar' || modalChartType === 'bar-cities') ? '950px' : '550px' }">
            ...
            <div v-else style="height: 400px;">
                <Bar v-if="modalChartType === 'bar-cities'" :data="chartCidadesFull" :options="{ responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, datalabels: { anchor: 'end', align: 'top', font: { weight: 'bold', size: 10 }, formatter: (v) => v.toFixed(2) } } }" />
                <Bar v-else-if="modalChartType === 'bar'" :data="chartEstadosFull" :options="{ responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, datalabels: { anchor: 'end', align: 'top', font: { weight: 'bold', size: 10 }, formatter: (v) => v.toFixed(2) } } }" />
                <Pie v-else-if="modalData" :data="{ labels: modalData.map(i => i.label), datasets: [{ data: modalData.map(i => i.frota), backgroundColor: orangePalette }] }" :options="{ responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom' }, datalabels: { display: true, formatter: (v, ctx) => ctx.chart.data.labels[ctx.dataIndex] + '\n' + formatNum(v) } } }" />
            </div>
        </div>
    </div>
  </div>
...
.modal-content { max-height: 100vh; display: flex; flex-direction: column; }
.modal-content > div:last-child { flex-grow: 1; min-height: 450px; }
</style>
